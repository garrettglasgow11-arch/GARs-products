...
(function modelPass() {
  ...
  function sculpt(rig) {
    ...
    /* The skull carries its own cheekbone and jaw.

       Both used to be lenses added on top, and an added mass on a curved
       surface always shows a rim where it lands, however shallow you make it
       — the cheeks read as two pale ovals stuck to the face at every setting
       tried. A head really is widest all the way round at the cheekbone and
       really does narrow to a jaw below it, so a surface of revolution can
       carry both, and then there is no rim to hide. */
    const SKULL = [
      [0.0001, -0.128 * S * H],
      [0.046 * S * H, -0.116 * S * H],   // chin
      [0.074 * S * H, -0.092 * S * H],   // jaw
      [0.090 * S * H, -0.060 * S * H],   // jaw angle
      [0.098 * S * H, -0.020 * S * H],
      [0.106 * S * H, 0.016 * S * H],    // cheekbone, at the eye line
      [0.112 * S * H, 0.034 * S * H],    // brow ridge
      [0.107 * S * H, 0.052 * S * H],    // set back above the brow
      [0.110 * S * H, 0.074 * S * H],    // widest, the parietal
      [0.100 * S * H, 0.112 * S * H],
      [0.072 * S * H, 0.140 * S * H],
      [0.0001, 0.156 * S * H]
    ];
    ...
    /* A head is markedly longer front-to-back than it is wide — about a
       quarter again. 1.07 was barely an oval. And it sits BEHIND the neck
       axis: the spine enters the skull well forward of the back of the
       cranium, so a head centred on its own neck has no occiput. */
    skull.scale.z = 1.16;
    skull.position.z = -0.010 * S;
    j.head.add(skull);
    rig.skull = skull;
    rig.__skull = { p: SKULL, y: SKY, z: -0.010 * S, sx: 1, sz: 1.16 * 0.96 };
  ...
  function buildFace(rig, j, m, S, H, cfg) {
    ...
    const SZ0 = SK.z || 0;                 // the skull's own z offset
    /* Place a mass by where its FRONT lands, not by a fraction of the surface
       depth. A fraction only works while every feature sits on the same part
       of the skull; once the landmarks moved down onto the narrower lower
       face, the same fractions left the cheeks, jaw and brow bulging off the
       head like barnacles. `proud` is how far the mass clears the skin, in
       head units, and it is the only number worth tuning. */
    const onFace = (mesh, y, x, halfDepth, proud) => {
      mesh.position.set(x, y, szAt(y, x) - halfDepth + proud);
      return mesh;
    };
    const szAt = (y, x) => {
      const r = surf(y);
      const q = Math.max(0, r * r - (x || 0) * (x || 0));
      return Math.sqrt(q) * SK.sz + SZ0;
    };
    const sz = (y) => szAt(y, 0);
    ...
    /* ------------------------------------------------------------------
       LANDMARKS, from the standard head canon rather than from taste.

       Three things were off, and they are the three everybody gets wrong:

       1. The eyes sat too high. They belong at the vertical midpoint of the
          WHOLE head — cranium included — not of the face. This skull runs
          -0.128 to +0.156, so the midline is +0.014. They were at +0.030,
          which is the classic error: it shrinks the forehead and makes the
          skull look like it has been sawn off.
       2. The face thirds were 0.024 / 0.066 / 0.124. Hairline-to-brow,
          brow-to-nose-base and nose-base-to-chin should be roughly EQUAL.
          The lower face was nearly twice its share, so the character read
          long-jawed no matter what the jaw geometry did.
       3. The ears floated. An ear spans exactly brow to nose base — that is
          what makes a head look assembled rather than decorated.

       These land at about 0.056 / 0.078 / 0.080, which is the canon with a
       slightly shorter forehead, because a bigger cranium is the stylised
       read this game wants.
       ------------------------------------------------------------------ */
    const browY = SK.y + 0.030 * U;
    const eyeY = SK.y + 0.016 * U;         // the head's own midline
    const noseY = SK.y - 0.048 * U;        // ear bottom lines up here
    const mouthY = SK.y - 0.078 * U;       // a third of the way nose to chin
    const chinY = SK.y - 0.112 * U;
    ...
    const chin = ball(0.046 * U, skin, 0, 1, FS);
    chin.scale.set(1.00, 0.80, 0.52);
    j.head.add(onFace(chin, chinY + 0.012 * U, 0, 0.0239 * U, 0.002 * U));
    ...
    for (const side of [-1, 1]) {
      ...
      /* Cheek, jaw and brow are all in the skull profile now. A supraorbital
         ridge does run right around the skull, so a surface of revolution can
         carry it, and the ridge that shades the eye no longer costs two pale
         ovals hanging over the eyebrows. Nothing is added here at all. */
    }

    // Nose: a bridge running down off the brow, then a tip.
    const bridge = ball(0.019 * U, skin, 0, 1, FS);
    bridge.scale.set(0.62, 1.55, 0.60);
    j.head.add(onFace(bridge, (browY + noseY) / 2 - 0.008 * U, 0, 0.0114 * U, 0.005 * U));
    const tip = ball(0.016 * U, skin, 0, 1, FS);
    tip.scale.set(1.34, 0.92, 1.05);
    j.head.add(onFace(tip, noseY - 0.008 * U, 0, 0.0168 * U, 0.015 * U));

    for (const side of [-1, 1]) {
      /* Span brow to nose base — 0.078 of a head unit here — and sit on the
         widest part of the skull, a little behind its midline. */
      const ear = ball(0.030 * U, skin, 0, 1, FS);
      ear.scale.set(0.30, 1.30, 0.80);
      ear.position.set(side * sx(eyeY) * 0.94, SK.y - 0.009 * U, SZ0 - 0.016 * U);
    ...
    /* An eye is a fifth of the head's width and the gap between the two is
       one more eye. That single ratio does more for a face than any amount
       of shaping. */
    const eyeR = 0.0225 * U;
    const eyeX = sx(eyeY) * 0.410;
    ...
    for (const side of [-1, 1]) {
      ...
      const sock = ball(eyeR * 1.10, dark, 0, 1, FS);
      sock.scale.set(1.04, 0.82, 0.34);
      sock.position.set(side * eyeX, eyeY, eyeZ - eyeR * 0.10);
      j.head.add(sock);

      const e = ball(eyeR, white, 0, 1, FS);
      e.scale.set(1.00, 0.88, 0.62);
    ...
    lips.scale.set(1.30, 0.62, 0.50);
    j.head.add(onFace(lips, mouthY, 0, 0.013 * U, 0.002 * U));
    const mz = lips.position.z;
    const line = ball(0.022 * U, rig.faceMouth || m.dark, 0, 1, FS);
    line.scale.set(1.34, 0.12, 0.44);
    line.position.set(0, mouthY, mz + 0.007 * U);
    j.head.add(line);
  ...
  function buildHair(rig, j, m, S, H, cfg) {
    ...
    j.head.add(lathe(cap, hairA, { squash: sq, pos: [0, 0, SZ0] }));
    ...
    const bk = lathe(back, hairA, {
      squash: sq, pos: [0, 0, SZ0], phiStart: Math.PI - 1.30, phiLength: 2.60
    });
    ...
    const shell = lathe(shellPts, hairA, {
      squash: sq, seg: Math.round(Sculpt.seg * 1.2), pos: [0, 0, SZ0],
    ...
    if (cfg.hairStyle === 'long') {
      for (const [x, rz, h] of [[-0.092, -0.22, 0.19], [0.092, 0.22, 0.19], [0, 0, 0.185]]) {
        ...
        root.position.set(x * U, SK.y + (back ? -0.010 : 0.020) * U,
          SZ0 + (back ? -(r(0.0) + T) * SK.sz * 0.86 : -0.030 * U));
        root.rotation.z = rz;
    ...
    for (const side of [-1, 1]) {
      const root = new THREE.Group();
      root.position.set(side * (r(0.030 * U) * 0.94), SK.y + 0.040 * U, SZ0 - 0.004 * U);
  ...
  function buildHelmet(rig, j, m, S, H, cfg) {
    ...
    j.head.add(lathe([
      ...
      [0.0001, SK.y + p[7][1] + T]
    ], m.armor, { squash: SK.sz / SK.sx, pos: [0, 0, SZ0] }));
    ...
    const rz = (y) => {
      ...
      for (let i = 1; i < p.length; i++)
        if (yl <= p[i][1]) {
          const t = (yl - p[i - 1][1]) / (p[i][1] - p[i - 1][1] || 1);
          return (p[i - 1][0] + (p[i][0] - p[i - 1][0]) * t) * SK.sz + SZ0;
        }
      return SZ0;
    };
